...
function cargarMensajesSoporte() {
  fetch('/api/mensajes-ayuda')
    ...
    .then(function (data) {
      mensajesSoporteGlobal = data.mensajes || [];
      renderizarTablaSoporte(mensajesSoporteGlobal);
    });
}

function renderizarTablaSoporte(mensajes) {
  var tbody = document.getElementById('tabla-soporte-body');
  var mensajeVacio = document.getElementById('mensaje-sin-soporte');
  ...
  mensajes.forEach(function (m) {
    ...
    var columnas = [
      ...
      m.categoria || '',
      m.mensaje
        ? m.mensaje.substring(0, 30) + (m.mensaje.length > 30 ? '...' : '')
        : '',
      m.fecha_creacion
        ? new Date(m.fecha_creacion).toLocaleString('es-BO')
        : '',
      m.estado || '',
    ];

    columnas.forEach(function (texto) {
    ...
    if (m.estado === 'pendiente') {
      ...
      btnResp.setAttribute('data-mensaje-id', m.id);
      btnResp.addEventListener('click', function () {
        abrirModalRespuesta(m.id);
      });
...
function configurarModalRespuesta() {
  ...
  document
    .getElementById('btn-cerrar-modal-respuesta')
    .addEventListener('click', cerrarModalRespuesta);
  document
    .getElementById('btn-cancelar-respuesta')
    .addEventListener('click', cerrarModalRespuesta);
  document
    .getElementById('form-respuesta-soporte')
    .addEventListener('submit', procesarRespuestaSoporte);

  modal.addEventListener('click', function (e) {
    if (e.target === modal) cerrarModalRespuesta();
  });
}

function abrirModalRespuesta(id) {
  var modal = document.getElementById('modal-respuesta-soporte');
  document.getElementById('mensaje-id-accion').value = id;
  document.getElementById('respuesta-soporte').value = '';
  modal.classList.add('panel-admin__modal-overlay--visible');
}
...
function procesarRespuestaSoporte() {
  var id = document.getElementById('mensaje-id-accion').value;
  var respuesta = document.getElementById('respuesta-soporte').value.trim();
  if (!respuesta) return;
  fetch('/api/mensajes-ayuda/' + id + '/responder', {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    ...
    .then(function (d) {
      if (d.error) {
        if (window.showToast) window.showToast(d.error, { tipo: 'error' });
        return;
      }
      if (window.showToast)
        window.showToast('Respuesta enviada', { tipo: 'success' });
